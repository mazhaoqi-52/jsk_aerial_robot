                               ...
                               boost::shared_ptr<aerial_robot_estimation::StateEstimator> estimator, double loop_du)
{
  ...
  simple_move_base_goal_sub_ = nh_.subscribe("/move_base_simple/goal", 1, &BaseNavigator::simpleMoveBaseGoalCallback,
...
#endif
  if (joy_udp_)
    joy_transport_hints = ros::TransportHints().udp();
...
void BaseNavigator::joyStickControl(const sensor_msgs::JoyConstPtr& joy_msg)
{
  ...
  if (getNaviState() == HOVER_STATE)
  {
    if (fabs(raw_z_cmd) > joy_stick_deadzone_)
    {
      ...
      // ROS_INFO("raw_z_cmd: %f, max_tleeop_z_vel: %f, target_vel_z: %f", raw_z_cmd, max_teleop_z_vel_,
      // target_vel_.z());
    }
    else
    {
  ...
  if (joy_cmd.buttons[PS3_BUTTON_REAR_LEFT_2])
  {
    ...
    tf::transformKDLToTF(segments_tf.at(baselink).Inverse() * segments_tf.at(teleop_local_frame_),
  ...
  switch (xy_control_mode_)
  {
    case POS_CONTROL_MODE: {
      if (fabs(raw_x_cmd) < joy_stick_deadzone_)
        raw_x_cmd = 0;
...
void BaseNavigator::generateNewTrajectory(geometry_msgs::PoseStamped pose)
{
  ...
  {
    ROS_WARN("[Nav] the target quaternion is invalid [%f, %f, %f, %f], reset as the start state", q.x(), q.y(), q.z(),
             q.w());
  ...
  ROS_INFO_STREAM("[Nav] revceive the new target pose of "
