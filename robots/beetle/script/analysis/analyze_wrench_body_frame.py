...
# CONSTANTS
# ------------------------------------------------------------------------
THRUST_MAX = 21.37  # Maximum thrust per rotor [N] (updated for current hardware)
THRUST_MIN = 0.0  # Minimum thrust per rotor [N]
WRENCH_ERROR_LIMIT = 1e-3  # Acceptable wrench error
TILT_LIMIT_DEG = 90.0  # Tilt angle limit [degrees], ±90° (updated for 180-degree range servos)


# ------------------------------------------------------------------------


def check_wrench_available(alloc_mtx, tgt_wrench, f_th_max=THRUST_MAX, wrench_error_limit=WRENCH_ERROR_LIMIT, tilt_limit_deg=TILT_LIMIT_DEG):
    """
    ...
    # constraints: each rotor's (Fx, Fy) magnitude ≤ f_th_max and tilt angle limit
    constraints = []
    for i in range(4):
        fx_idx = 2 * i
        fy_idx = 2 * i + 1
        ...
        # Tilt angle constraint: Fy ≥ 0 (rotor can only push, not pull)
        constraints.append(x[fy_idx] >= 0)
        
        # For tilt_limit_deg < 90°: |Fx| ≤ Fy * tan(limit)
...
if __name__ == "__main__":
    ...
    if mode == "force":
        ...
        fig.savefig("body_force_3d.png", bbox_inches="tight", pad_inches=0.3, dpi=150)
        ...
        fig2.savefig("body_force_xz_projection.png", bbox_inches="tight", pad_inches=0.3, dpi=150)
        plt.show()
        ...
        ax.set_box_aspect((np.ptp(endpoints[:, 0]), np.ptp(endpoints[:, 1]), np.ptp(endpoints[:, 2])))
        fig.savefig("body_torque_3d.png", bbox_inches="tight", pad_inches=0.3, dpi=150)
        ...
        fig2.savefig("body_torque_xz_projection.png", bbox_inches="tight", pad_inches=0.3, dpi=150)
        plt.show()
