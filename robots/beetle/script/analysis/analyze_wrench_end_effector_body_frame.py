...
Compute maximum force or torque at End Effector position in the body frame
for n-module assembled omnidirectional aerial vehicles.

The key difference from analyze_wrench_assembled_body_frame.py:
...
- This means for force analysis: max EE force = max CoG force (force is translation-invariant)
- For torque analysis: we must account for the lever arm effect

Usage examples:
    python analyze_wrench_end_effector_body_frame.py                           # Use config file
    python analyze_wrench_end_effector_body_frame.py --ee_module 1             # EE on module 1
    python analyze_wrench_end_effector_body_frame.py -n 3 --ee_module 2        # 3 modules, EE on last
"""
...
def check_ee_wrench_available(
    ...
    The relation between EE wrench and CoG wrench:
        F_cog = F_ee
        τ_cog = τ_ee + r_ee × F_ee
        
    Where r_ee is the position vector from CoG to EE.
    ...
    # Objective: minimize squared error ||A x - tgt_cog_wrench||^2
...
def find_max_ee_wrench_for_orientation_body(
    alloc_mtx,
    n_rotors,
    r_ee,
    roll_deg,
    ...
    along a specific direction in body frame.

    Parameters:
        alloc_mtx: allocation matrix (6 × 2*n_rotors)
        n_rotors: total number of rotors
        r_ee: position of EE relative to CoG [x, y, z]
        roll_deg, pitch_deg, yaw_deg: Euler angles defining the direction
    ...
    for iter_idx in range(max_iters):
        ...
        # Build target EE wrench: [Fx; Fy; Fz; τx; τy; τz]
        ...
        ok, err = check_ee_wrench_available(
            alloc_mtx, tgt_ee_wrench, n_rotors, r_ee,
            f_th_max=f_th_max, tilt_limit_deg=tilt_limit_deg
        )
        if ok:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze maximum force or torque at End Effector in the BODY frame.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python analyze_wrench_end_effector_body_frame.py                      # Use default config
  python analyze_wrench_end_effector_body_frame.py -m force --ee_module 0
  python analyze_wrench_end_effector_body_frame.py -n 3 --ee_module 2   # EE on last module
        """
    )
    parser.add_argument(
        "config",
    ...
    print("=" * 60)
    print("End Effector Wrench Analysis (Body Frame)")
    print("=" * 60)
    ...
    for i, pitch_deg in enumerate(pitch_list):
        for j, yaw_deg in enumerate(yaw_list):
            max_val, _ = find_max_ee_wrench_for_orientation_body(
                alloc_mat,
                n_rotors,
                r_ee,
                roll_deg=0.0,
        ...
        print(f"[EE Body {mode.capitalize()}] Completed pitch = {pitch_deg:.1f}°")

    # Save to file
    if save_to_npz:
        filename = f"ee_body_{mode}_map_{n_modules}modules_ee{ee_module}.npz"
        np.savez(
            filename,
            yaw_list=yaw_list,
    ...
    title = f"Max EE {mode.capitalize()} in Body Frame\n"
    title += f"({n_modules} module{'s' if n_modules > 1 else ''}, EE on module {ee_module})"
    ax.set_title(title)
    
    fig_filename = f"ee_body_{mode}_{n_modules}modules_ee{ee_module}_tilt{int(tilt_limit_deg)}_3d.png"
    ...
    fig2_filename = f"ee_body_{mode}_{n_modules}modules_ee{ee_module}_tilt{int(tilt_limit_deg)}_xz.png"
