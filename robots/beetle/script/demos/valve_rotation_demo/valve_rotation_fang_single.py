...
class SingleUAVStateBase(smach.State):
    # Shared target Z from Phase 3 for later states.
    ...
    def _save_insertion_geometry(self, current_position):
        ...
        rospy.loginfo(f"[OK] Saved insertion geometry: spoke_angle={math.degrees(spoke_angle):.1f} deg, distance={initial_distance:.3f}m")
    ...
    def _calculate_rotated_beam_position(self, current_valve_yaw=None):
        ...
        rospy.loginfo(f"[OK] Beam rotated by {math.degrees(valve_rotation):.1f} deg -> "
        ...
        while rospy.get_time() - start_time < duration and not rospy.is_shutdown():
            ...
            if int(elapsed) != int(elapsed - 0.1):  # Log approximately every second
                ...
                if current_pos is not None:
                    ...
                    rospy.loginfo(f"  Stabilization progress: {elapsed:.1f}s, pos_err={pos_error*1000:.1f}mm, yaw_err={math.degrees(yaw_error):.1f} deg")
    ...
    def _replan_trajectory_from_contact(self, contact_position, contact_yaw, valve_center, rotation_angle, remaining_duration):
        ...
        try:
            ...
            if new_trajectory is not None:
                rospy.loginfo("[OK] Trajectory successfully replanned from stable contact position")
            else:
                rospy.logerr("[FAIL] Failed to create replanned trajectory")
        ...
        if self.last_position is not None:
            ...
            if position_stuck and yaw_stuck:
                if self.stuck_start_time is None:
                    ...
                    rospy.loginfo("[WARN] UAV movement below threshold - starting stuck detection timer")
                else:
    ...
    def execute_safe_movement_to_position(self, target_pos, target_yaw):
        ...
        try:
            ...
            rospy.loginfo(f"Current yaw: {math.degrees(self.current_yaw):.1f} deg")
            ...
            rospy.loginfo(f"Required yaw change: {math.degrees(yaw_change):.1f} deg")
            ...
            if abs(yaw_change) > 0.05:  # Only if significant yaw change needed
                rospy.loginfo(f"Performing yaw adjustment: {math.degrees(yaw_change):.1f} deg")
            else:
            ...
            rospy.loginfo("[OK] THREE-STAGE MOVEMENT COMPLETED SUCCESSFULLY")
            return True
    ...
    def execute_integrated_three_stage_insertion(self, start_pos, target_pos, target_yaw,
        ...
        rospy.loginfo(f"Valve yaw: {math.degrees(valve_yaw_target):.1f} deg")
        rospy.loginfo(f"Current yaw: {math.degrees(self.current_yaw):.1f} deg")
        valve_yaw_change = abs(self.normalize_angle(valve_yaw_target - self.current_yaw))
        rospy.loginfo(f"Required valve yaw change: {math.degrees(valve_yaw_change):.1f} deg")
        ...
        stage1_success = self.motion_controller.execute_progressive_precision_trajectory(
            ...
            final_yaw_threshold=self.yaw_precision_levels['fine']   # Enhanced: 0.86 deg precision (was 0.015)
        )
        ...
        rospy.loginfo("[OK] Stage 1 completed: Valve yaw alignment successful")
        ...
        rospy.loginfo(f"  Target valve yaw: {math.degrees(valve_yaw_target):.3f} deg")
        rospy.loginfo(f"  Actual yaw: {math.degrees(current_yaw):.3f} deg")
        rospy.loginfo(f"  Valve yaw error: {math.degrees(valve_yaw_error):.3f} deg")
        rospy.loginfo(f"  Required precision: {math.degrees(0.02):.3f} deg")

        # ENHANCED: Validation for Stage 1 valve yaw alignment
        if valve_yaw_error > 0.05236:  # 3.0 deg tolerance for preparation phase (tightened)
            rospy.logwarn(f"Stage 1 valve yaw alignment insufficient: {math.degrees(valve_yaw_error):.3f} deg > {math.degrees(0.05236):.3f} deg")
            rospy.logwarn("UAV is not properly aligned with valve for consistent approach")
        else:
            rospy.loginfo(f"[OK] Stage 1 valve yaw alignment excellent: {math.degrees(valve_yaw_error):.3f} deg <= {math.degrees(0.05236):.3f} deg")
        ...
        rospy.loginfo(f"Planned Stage 1 end: ({start_pos[0]:.3f}, {start_pos[1]:.3f}, {start_pos[2]:.3f}) at {math.degrees(valve_yaw_target):.1f} deg")
        rospy.loginfo(f"Actual Stage 1 end: ({stage1_actual_pos[0]:.3f}, {stage1_actual_pos[1]:.3f}, {stage1_actual_pos[2]:.3f}) at {math.degrees(stage1_actual_yaw):.1f} deg")
        ...
        rospy.loginfo(f"Stage 1 deviations: Position {stage1_pos_deviation*1000:.1f}mm, Yaw {math.degrees(stage1_yaw_deviation):.2f} deg")

        if stage1_pos_deviation > 0.025 or stage1_yaw_deviation > 0.05236:  # Tightened thresholds: 25mm position, 3.0 deg yaw
            ...
            correction_success = self.motion_controller.execute_progressive_precision_trajectory(
                ...
                final_pos_threshold=0.015, final_yaw_threshold=0.02  # 15mm, 1.1 deg final precision
            )
            if correction_success:
                rospy.loginfo("[OK] Stage 1 position correction successful")
                # Re-acquire corrected position
                stage1_actual_pos = self.get_current_position()
                stage1_actual_yaw = self.get_current_yaw()
            else:
                rospy.logwarn("[WARN] Stage 1 position correction failed - continuing with replanning")
        else:
        ...
        rospy.loginfo(f"  Using actual yaw: {math.degrees(aligned_yaw):.1f} deg (valve yaw locked)")
        ...
        rospy.loginfo("[OK] Stage 2 completed: Position move with valve yaw locked successful")
        ...
        rospy.loginfo(f"Planned Stage 2 end: ({intermediate_pos[0]:.3f}, {intermediate_pos[1]:.3f}, {intermediate_pos[2]:.3f}) at {math.degrees(aligned_yaw):.1f} deg")
        rospy.loginfo(f"Actual Stage 2 end: ({stage2_actual_pos[0]:.3f}, {stage2_actual_pos[1]:.3f}, {stage2_actual_pos[2]:.3f}) at {math.degrees(stage2_actual_yaw):.1f} deg")
        ...
        rospy.loginfo(f"Stage 2 deviations: Position {stage2_pos_deviation*1000:.1f}mm, Yaw {math.degrees(stage2_yaw_deviation):.2f} deg")

        if stage2_pos_deviation > 0.035 or stage2_yaw_deviation > 0.05236:  # Tightened thresholds: 35mm position, 3.0 deg yaw
            ...
            correction_success = self.motion_controller.execute_progressive_precision_trajectory(
                ...
                final_pos_threshold=0.020, final_yaw_threshold=0.02  # 20mm, 1.1 deg final precision
            )
            if correction_success:
                rospy.loginfo("[OK] Stage 2 position correction successful")
                # Re-acquire corrected position
                stage2_actual_pos = self.get_current_position()
                stage2_actual_yaw = self.get_current_yaw()
            else:
                rospy.logwarn("[WARN] Stage 2 position correction failed - continuing with replanning")
        else:
        ...
        if xy_distance > 0.01:  # 10mm threshold
            ...
            if not self.execute_xy_only_movement(intermediate_actual_pos, xy_target, current_yaw):
                ...
                return False
            rospy.loginfo("[OK] Phase 3A completed: XY positioning successful with valve yaw locked")
        else:
        ...
        rospy.loginfo(f"Planned Phase 3A end: ({xy_target[0]:.3f}, {xy_target[1]:.3f}, {xy_target[2]:.3f}) at {math.degrees(current_yaw):.1f} deg")
        rospy.loginfo(f"Actual Phase 3A end: ({phase3a_actual_pos[0]:.3f}, {phase3a_actual_pos[1]:.3f}, {phase3a_actual_pos[2]:.3f}) at {math.degrees(phase3a_actual_yaw):.1f} deg")
        ...
        rospy.loginfo(f"Phase 3A deviations: Position {phase3a_pos_deviation*1000:.1f}mm, Yaw {math.degrees(phase3a_yaw_deviation):.2f} deg")
        ...
        rospy.loginfo(f"  Current yaw (actual from 3A): {math.degrees(current_yaw):.3f} deg")
        rospy.loginfo(f"  Target yaw (spoke aligned): {math.degrees(target_yaw):.3f} deg")
        rospy.loginfo(f"  Required spoke alignment rotation: {math.degrees(yaw_error):.3f} deg")
        rospy.loginfo(f"  Precision threshold: {math.degrees(0.02):.3f} deg")

        # CRITICAL: This is the spoke gap alignment phase (AFTER XY positioning)
        if yaw_error > 0.02:  # 1.15 deg threshold for precise spoke insertion
            rospy.loginfo(f"[OK] SPOKE GAP ALIGNMENT needed: rotating {math.degrees(yaw_error):.3f} deg for insertion")
            if not self.execute_yaw_adjustment_with_compensation(target_yaw):
                rospy.logerr("Phase 3B: Spoke gap yaw alignment failed")
                return False
            rospy.loginfo("[OK] Phase 3B completed: Spoke gap yaw alignment successful")
        else:
            rospy.loginfo(f"[OK] Spoke gap already aligned within {math.degrees(0.01):.3f} deg tolerance")
        ...
        rospy.loginfo(f"Target Phase 3B end: spoke yaw {math.degrees(target_yaw):.1f} deg")
        rospy.loginfo(f"Actual Phase 3B end: ({phase3b_actual_pos[0]:.3f}, {phase3b_actual_pos[1]:.3f}, {phase3b_actual_pos[2]:.3f}) at {math.degrees(phase3b_actual_yaw):.1f} deg")

        # Calculate Phase 3B yaw deviation
        phase3b_yaw_deviation = abs(self.normalize_angle(phase3b_actual_yaw - target_yaw))
        rospy.loginfo(f"Phase 3B yaw deviation: {math.degrees(phase3b_yaw_deviation):.2f} deg")
        ...
        if z_distance > 0.01:  # 10mm threshold
            ...
            if not self.execute_z_only_movement(current_pos_after_yaw, z_target):
                ...
                return False
            rospy.loginfo("[OK] Phase 3C completed: Z insertion successful into spoke gaps")
        else:
        ...
        if final_actual_pos is not None:
            ...
            rospy.loginfo(f"Original target: ({target_pos[0]:.3f}, {target_pos[1]:.3f}, {target_pos[2]:.3f}) at {math.degrees(target_yaw):.1f} deg")
            rospy.loginfo(f"Final actual: ({final_actual_pos[0]:.3f}, {final_actual_pos[1]:.3f}, {final_actual_pos[2]:.3f}) at {math.degrees(final_actual_yaw):.1f} deg")
            ...
            if final_pos_error < 0.02:  # 20mm tolerance
                rospy.loginfo("[OK] ADAPTIVE INTEGRATED THREE-STAGE INSERTION SUCCESSFUL")
                return True
            else:
        ...
        rospy.loginfo("[OK] Stage 3 completed: Precision insertion successful")
        ...
        if final_actual_pos is not None:
            ...
            rospy.loginfo(f"  Stage 1 deviation: {stage1_pos_deviation*1000:.1f}mm, {math.degrees(stage1_yaw_deviation):.2f} deg")
            rospy.loginfo(f"  Stage 2 deviation: {stage2_pos_deviation*1000:.1f}mm, {math.degrees(stage2_yaw_deviation):.2f} deg")
            rospy.loginfo(f"  Phase 3A deviation: {phase3a_pos_deviation*1000:.1f}mm, {math.degrees(phase3a_yaw_deviation):.2f} deg")
            rospy.loginfo(f"  Phase 3B deviation: {math.degrees(phase3b_yaw_deviation):.2f} deg")
            rospy.loginfo(f"  Final accuracy: {final_pos_error*1000:.1f}mm, {math.degrees(final_yaw_error):.2f} deg")
        ...
        rospy.loginfo("[OK] ADAPTIVE INTEGRATED THREE-STAGE INSERTION COMPLETED (with warnings)")
        return True
    ...
    def execute_yaw_adjustment_with_compensation(self, target_yaw):
        ...
        max_attempts = 3
        yaw_threshold = self.yaw_precision_levels['moderate']  # Enhanced: 1.15 deg precision (was 0.02)
        ...
        for attempt in range(max_attempts):
            ...
            rospy.loginfo(f"Attempt {attempt+1}: Yaw error = {math.degrees(abs(yaw_diff)):.2f} deg")

            # Check if precision requirements are already met
            if abs(yaw_diff) <= yaw_threshold:
                rospy.loginfo(f"[OK] Yaw already within tolerance ({math.degrees(abs(yaw_diff)):.3f} deg <= {math.degrees(yaw_threshold):.3f} deg)")
                return True

            rospy.loginfo(f"Yaw adjustment needed: {math.degrees(yaw_diff):.2f} deg")
            ...
            success = self.motion_controller.execute_progressive_precision_trajectory(
                ...
                final_yaw_threshold=yaw_threshold    # 0.57 deg yaw precision
            )
            ...
            rospy.loginfo(f"Attempt {attempt+1}: Final yaw error = {math.degrees(final_yaw_error):.3f} deg")

            if final_yaw_error <= yaw_threshold:
                rospy.loginfo("[OK] Yaw adjustment completed with compensation")
                return True
            else:
                rospy.logwarn(f"Attempt {attempt+1}: Yaw error {math.degrees(final_yaw_error):.3f} deg exceeds threshold {math.degrees(yaw_threshold):.3f} deg")

        rospy.logerr(f"[FAIL] Yaw adjustment failed after {max_attempts} attempts")
        return False
    ...
    def execute_xy_only_movement(self, start_pos, target_pos, locked_yaw):
        ...
        for attempt in range(max_attempts):
            ...
            if xy_distance <= position_threshold:
                rospy.loginfo(f"[OK] XY position already within tolerance ({xy_distance*1000:.1f}mm <= {position_threshold*1000:.1f}mm)")
                return True
            ...
            if is_simulation:
                ...
                yaw_thresh = 0.1   # Relaxed yaw precision (5.7 deg)
            ...
            rospy.loginfo(f"Attempt {attempt+1}: Final errors - XY: {final_xy_error*1000:.1f}mm, Z drift: {final_z_drift*1000:.1f}mm, Yaw drift: {math.degrees(final_yaw_drift):.2f} deg")
            ...
            if is_simulation:
                ...
                yaw_drift_threshold = 0.05236  # 3.0 deg yaw drift tolerance (tightened)
            else:
            ...
            if final_xy_error <= xy_threshold:
                if final_z_drift <= z_drift_threshold:
                    if final_yaw_drift <= yaw_drift_threshold:
                        rospy.loginfo("[OK] XY positioning completed with Z and yaw locked")
                        if not is_simulation:
                            rospy.loginfo(f"[OK] Real machine tolerances applied: XY+/-{xy_threshold*1000:.0f}mm, Z+/-{z_drift_threshold*1000:.0f}mm, Yaw+/-{math.degrees(yaw_drift_threshold):.1f} deg")
                        return True
                    else:
                        rospy.logwarn(f"Attempt {attempt+1}: Excessive yaw drift {math.degrees(final_yaw_drift):.2f} deg")
                else:
        ...
        rospy.logerr(f"[FAIL] XY positioning failed after {max_attempts} attempts")
        return False
    ...
    def execute_z_only_movement(self, start_pos, target_pos):
        ...
        for attempt in range(max_attempts):
            ...
            if z_distance <= position_threshold:
                rospy.loginfo(f"[OK] Z position already within tolerance ({z_distance*1000:.1f}mm <= {position_threshold*1000:.1f}mm)")
                return True
            ...
            success = self.motion_controller.execute_vel_accel_trajectory(
                ...
                yaw_threshold=self.yaw_precision_levels['fine'],  # Enhanced: 0.86 deg precision (was 0.01/0.57 deg)
            ...
            rospy.loginfo(f"Attempt {attempt+1}: Final errors - Z: {final_z_error*1000:.1f}mm, XY drift: {final_xy_drift*1000:.1f}mm, Yaw drift: {math.degrees(final_yaw_drift):.2f} deg")

            if final_z_error <= position_threshold:
                if final_xy_drift <= 0.012:  # 12mm XY drift tolerance
                    if final_yaw_drift <= 0.05236:  # 3.0 deg yaw drift tolerance (tightened)
                        rospy.loginfo("[OK] Z insertion completed with XY and yaw locked")
                        return True
                    else:
                        rospy.logwarn(f"Attempt {attempt+1}: Excessive yaw drift {math.degrees(final_yaw_drift):.2f} deg")
                else:
        ...
        rospy.logerr(f"[FAIL] Z insertion failed after {max_attempts} attempts")
        return False
...
class MoveToValveState(SingleUAVStateBase):
    def __init__(self, module_id=1):
        ...
        self.final_yaw_threshold = 0.0175  # 1 deg final yaw accuracy (tightened to meet 1 deg requirement)

        # Position and orientation thresholds
        self.pos_threshold = 0.05
        self.yaw_threshold = 0.05236  # Tightened to 3 deg
    ...
    def _execute_phase1_yaw_adjustment(self, current_position, target_yaw):
        ...
        # Log yaw adjustment
        initial_yaw = self.current_yaw
        rospy.loginfo(f"Phase 1: Adjusting yaw from {math.degrees(initial_yaw):.1f} deg to {math.degrees(target_yaw):.1f} deg")
        ...
        yaw_converged = self.wait_for_yaw_convergence(
            target_yaw,
            yaw_thresh=0.0524,  # 3 deg threshold
    ...
    def _execute_phase3_z_descent_and_adjustment(self, final_target, final_yaw):
        ...
        if current_yaw is not None:
            ...
            major_yaw_needed = yaw_error > 0.175  # 10 deg threshold for major adjustment
            minor_yaw_needed = yaw_error > 0.0175  # 1 deg threshold for minor adjustment
        ...
        # Phase 3A: Major yaw adjustment if needed (>10 deg)
        if major_yaw_needed:
            rospy.loginfo("=== Phase 3A: Major yaw adjustment ===")
            # Calculate intermediate yaw (leave ~8 deg for fine adjustment during descent)
            yaw_diff = self.normalize_angle(final_yaw - current_yaw)
            intermediate_yaw = current_yaw + yaw_diff * 0.85  # Complete 85% of rotation

            rospy.loginfo(f"Adjusting yaw from {math.degrees(current_yaw):.1f} deg to {math.degrees(intermediate_yaw):.1f} deg")
            ...
            success = self.active_position_convergence(
                ...
                yaw_thresh=0.0524, # 3 deg yaw tolerance (looser for major adjustment phase)
                timeout=20.0,      #  Reduced timeout for faster overall completion
                max_yaw_step=math.radians(3.5)  #  3.5 deg/step ~ 7 deg/s effective speed (accounting for delays)
            )

            if not success:
                rospy.logwarn("Phase 3A: Major yaw adjustment did not fully converge, continuing")
            else:
                rospy.loginfo("[OK] Phase 3A completed: Major yaw adjustment successful")
    ...
    def _execute_controlled_z_descent(self, start_pos, final_target, final_yaw, descent_speed=0.12):
        ...
        if current_yaw is not None:
            ...
            rospy.loginfo(f"Remaining yaw error: {math.degrees(remaining_yaw_error):.2f} deg")

            if abs(remaining_yaw_error) > 0.0175:  # 1 deg threshold
                rospy.loginfo("=== Phase 3B-1: Precision yaw adjustment ===")
                rospy.loginfo(f"Adjusting yaw from {math.degrees(current_yaw):.1f} deg to {math.degrees(final_yaw):.1f} deg")
                ...
                if current_pos is not None:
                    ...
                    yaw_converged = self.active_position_convergence(
                        ...
                        yaw_thresh=0.0175,              # 1 deg tolerance for yaw
                        timeout=15.0,                   # Reasonable timeout for precision phase
                        max_yaw_step=math.radians(2.0)  # 2 deg/step ~ 4 deg/s for fine adjustment phase
                    )
                else:
                ...
                if not yaw_converged:
                    ...
                    rospy.loginfo("[OK] Phase 3B-1 completed: Formation-style yaw precision adjustment successful")
                ...
                rospy.loginfo("Phase 3B-1 skipped: Yaw already within 1 deg tolerance")
        ...
        for step in range(num_steps):
            ...
            step_yaw_thresh = 0.0175  # 1 deg - tight since yaw should be stable
            ...
            rospy.loginfo(f"  Threshold: pos={step_pos_thresh*1000:.1f}mm, yaw={math.degrees(step_yaw_thresh):.1f} deg")
            ...
            if current_check_pos is not None:
                ...
                if xy_error > 0.025:  # 25mm XY error threshold
                    ...
                    if xy_corrected:
                        rospy.loginfo("[OK] XY correction successful")
                    else:
                        rospy.logwarn("[WARN] XY correction timeout, but continuing")
        ...
        final_success = self.active_position_convergence(
            ...
            yaw_thresh=0.0175, # 1 deg final yaw precision
        ...
        if current_pos is not None:
            ...
            if final_check_pos is not None:
                ...
                if final_xy_error > 0.020:  # 20mm threshold
                    rospy.logwarn(f"[WARN] Final XY error large: {final_xy_error*1000:.1f}mm - valve insertion may be imprecise")
                else:
                    rospy.loginfo(f"[OK] Final XY positioning acceptable: {final_xy_error*1000:.1f}mm")
            else:
        ...
        if final_success:
            rospy.loginfo("[OK] SEPARATED CONTROL SUCCESSFUL: Z descent completed with precision")
        else:
            rospy.logwarn("[WARN] Separated control completed but final convergence not fully achieved")
    ...
    def generate_polynomial_trajectory(self, start_pos, target_pos, target_yaw, num_points=20):
        ...
        rospy.loginfo(f"Trajectory parameters: distance={total_distance:.3f}m, angle_change={math.degrees(yaw_change):.1f} deg")
        ...
        for i in range(num_points + 1):  # +1 to include final target
            ...
            # Velocity = dp/dt = (dp/dtau) * (dtau/dt) = (dp/dtau) / duration
            ...
            if i % 5 == 0:  # Log every 5th point
                rospy.loginfo(f"Point{i}: pos=[{pos_x:.3f}, {pos_y:.3f}, {pos_z:.3f}], "
                            f"yaw={math.degrees(yaw):.1f} deg, vel={vel_magnitude:.3f}m/s (factor={velocity_factor:.2f})")
    ...
    def generate_smooth_waypoint_trajectory(self, start_pos, target_pos, target_yaw, num_waypoints=7):
        ...
        for i in range(num_waypoints + 1):  # +1 to include final target
            ...
            rospy.loginfo(f"Waypoint {i}: pos=({interp_pos[0]:.3f}, {interp_pos[1]:.3f}, {interp_pos[2]:.3f}), "
                         f"yaw={math.degrees(interp_yaw):.1f} deg, vel_mag={np.linalg.norm(velocity):.2f}m/s")
    ...
    def execute_polynomial_trajectory(self, trajectory_points):
        ...
        for i, (target_pos, target_yaw, target_vel, vel_magnitude) in enumerate(trajectory_points):
            ...
            rospy.loginfo(f"Executing point {i+1}/{len(trajectory_points)}: "
                         ...
                         f"yaw={math.degrees(target_yaw):.1f} deg, vel={vel_magnitude:.3f}m/s")
            ...
            if is_final_point:
                ...
                yaw_thresh = self.final_yaw_threshold  # 1 deg
                ...
                if is_final_precision_point:
                    ...
                    intermediate_yaw_thresh = 0.1   # 6 deg航向要求
                    ...
                    intermediate_yaw_thresh = 0.25  #  14 deg宽松航向要求
    ...
    def active_position_convergence(self, target_pos, target_yaw, pos_thresh=0.025, yaw_thresh=0.0175, timeout=15.0, max_yaw_step=None, max_z_step=None):
        ...
        rospy.loginfo(f"Thresholds: pos={pos_thresh*1000:.1f}mm, yaw={math.degrees(yaw_thresh):.1f} deg, consecutive={required_consecutive}")
        ...
        while rospy.get_time() - start_time < timeout:
            ...
            if position_ok and yaw_ok:
                ...
                if max_yaw_step is not None and max_yaw_step > 0.0:
                    ...
                    if abs(yaw_delta) > max_yaw_step:
                        ...
                        if int(elapsed * 5.0) % 20 == 0:  # Every 4 seconds
                            rospy.loginfo(f"[YawLimit] Step limited to {math.degrees(yaw_delta):.1f} deg "
                                        f"(remaining {math.degrees(abs(self.normalize_angle(target_yaw - current_yaw))):.1f} deg)")
                ...
                max_angular_vel = 0.05  # 0.05 rad/s ~ 2.9 deg/s，更保守的角速度限制
                ...
                if int(elapsed * 0.5) % 1 == 0:  # Every 2 seconds
                    rospy.loginfo(f"Active convergence progress: pos_err={pos_error*1000:.1f}mm, "
                                f"yaw_err={math.degrees(yaw_error):.1f} deg, time={elapsed:.1f}s")
        ...
        if current_pos is not None and current_yaw is not None:
            ...
            rospy.logwarn(f"Active convergence timeout: final errors pos={final_pos_error*1000:.1f}mm, "
                         f"yaw={math.degrees(final_yaw_error):.1f} deg")
    ...
    def wait_for_yaw_convergence(self, target_yaw, yaw_thresh=0.0524, timeout=10.0):
        ...
        rospy.loginfo(f"Waiting for yaw convergence - target={math.degrees(target_yaw):.1f} deg, thresh={math.degrees(yaw_thresh):.1f} deg")
        ...
        while not rospy.is_shutdown():
            ...
            if int((rospy.get_time() - start_time) * 10) % 10 == 0:
                rospy.loginfo(f"Yaw convergence: current={math.degrees(current_yaw):.1f} deg, target={math.degrees(target_yaw):.1f} deg, error={math.degrees(yaw_error):.1f} deg")
            ...
            if int(elapsed) % 2 == 0 and elapsed > 0:
                rospy.loginfo_throttle(2, f"Yaw convergence progress: error={math.degrees(yaw_error):.2f} deg, time={elapsed:.1f}s")
    ...
    def wait_for_final_convergence(self, target_pos, target_yaw, pos_thresh=0.025, yaw_thresh=0.0524, timeout=8.0):
        ...
        while not rospy.is_shutdown():
            ...
            if pos_error < pos_thresh and yaw_error < yaw_thresh:
                consecutive_good_readings += 1
                if consecutive_good_readings >= required_consecutive:
                    rospy.loginfo(f"Final convergence achieved! Position error: {pos_error*1000:.1f}mm, Yaw error: {math.degrees(yaw_error):.2f} deg")
                    return True
            else:
                consecutive_good_readings = 0
            ...
            elapsed = rospy.get_time() - start_time
            if int(elapsed) % 1 == 0:
                rospy.loginfo_throttle(1, f"Final convergence progress: pos_err={pos_error*1000:.1f}mm, yaw_err={math.degrees(yaw_error):.2f} deg")
    ...
    def execute_smooth_trajectory(self, waypoints):
        ...
        for i, waypoint in enumerate(waypoints):
            ...
            if is_final_waypoint:
                ...
                yaw_thresh = self.final_yaw_threshold  # 3 deg
                ...
                yaw_thresh = 0.0873  # 5 deg (more relaxed)
        ...
        total_time = rospy.get_time() - start_time
        rospy.loginfo(f"[OK] 平滑轨迹执行完成，总用时: {total_time:.2f}s")
        return True
    ...
    def wait_for_waypoint_convergence(self, target_pos, target_yaw, pos_thresh, yaw_thresh, timeout):
        ...
        if current_pos is not None:
            ...
            rospy.logwarn(f"Waypoint收敛超时: pos_error={pos_error*1000:.1f}mm, yaw_error={math.degrees(yaw_error):.1f} deg")
...
class DescendAndContactState(SingleUAVStateBase):
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo(f"当前偏航: {math.degrees(current_yaw):.1f} deg")
        ...
        rospy.loginfo("[OK] 圆周接触建立，检测到阀门转动")
    ...
    def _execute_linear_insertion_phase(self):
        ...
        rospy.loginfo(f"目标偏航: {math.degrees(target_yaw):.1f} deg")
    ...
    def _execute_circular_contact_phase(self, userdata):
        ...
        rospy.loginfo(f"初始阀门角度: {math.degrees(initial_valve_yaw):.1f} deg")
        ...
        trajectory_generator = OnlineCircularTrajectoryGenerator(
            ...
            target_angular_velocity=0.1,  # 0.1 rad/s ~ 5.7 deg/s（Contact阶段需要足够速度建立接触）
        ...
        valve_rotation_threshold = 0.1  # Dragon标准：0.1 rad ~ 5.7 deg（移除强制时间约束）
        ...
        rospy.loginfo(f"开始圆周接触运动 - 目标检测: {math.degrees(valve_rotation_threshold):.1f} deg阀门转动（Dragon逻辑）")
        ...
        while (rospy.Time.now().to_sec() - start_time) < contact_duration:
            ...
            if valve_rotation > valve_rotation_threshold:
                ...
                rospy.loginfo(f"[OK] 检测到阀门转动: {math.degrees(valve_rotation):.1f} deg "
                            f"(阈值: {math.degrees(valve_rotation_threshold):.1f} deg)")
            ...
            if current_time - start_time > 1.0:  # 1秒后开始输出调试信息
                rospy.loginfo_throttle(2.0,
                    ...
                    f"角速度={math.degrees(state_info['angular_velocity']):.1f} deg/s, "
                    f"阀门转动={math.degrees(valve_rotation):.2f} deg, "
        ...
        if final_valve_rotation > math.radians(0.5):  # 降低阈值到0.5 deg
            rospy.logwarn(f"接触超时，但检测到微小阀门转动: {math.degrees(final_valve_rotation):.2f} deg，认为成功")
            self._set_output_data(initial_valve_yaw, current_pos, current_yaw, userdata)
            return 'success'
    ...
    def _set_output_data(self, initial_valve_yaw, current_pos, current_yaw, userdata,
        ...
        if trajectory_generator is not None:
            ...
            rospy.loginfo("[OK] 轨迹生成器状态已传递给Manipulate阶段")
        else:
        ...
        rospy.loginfo("[OK] Contact模式输出数据设置完成")

        rospy.loginfo("[OK] 阀门接触检测完成，准备开始转动")
        return 'succeeded'
    ...
    def determine_optimal_contact_point(self):
        ...
        if strategy_type == 'two_phase_safe_insertion':
            ...
            rospy.loginfo(f"Left claw -> {strategy.get('left_gap', 'N/A')} at {math.degrees(strategy.get('left_gap_angle', 0)):.0f} deg")
            rospy.loginfo(f"Right claw -> {strategy.get('right_gap', 'N/A')} at {math.degrees(strategy.get('right_gap_angle', 0)):.0f} deg")
    ...
    def rotate_and_insert_to_beam_gap(self, contact_point):
        ...
        if success:
            rospy.loginfo("[OK] Successfully executed dual-fang insertion strategy")
            return True
        else:
            rospy.logerr("[FAIL] Failed to execute dual-fang insertion")
            return False
    ...
    def move_along_valve_circumference(self, contact_point):
        ...
        if distance_to_optimal_contact < CONTACT_TOLERANCE:
            rospy.loginfo("[OK] Already positioned at 55mm optimal contact point - circumferential movement complete")
            return True
        else:
    ...
    def prepare_for_valve_rotation_at_insertion_point(self):
        ...
        self.valve_yaw_change_threshold = math.radians(2.0)  # 2 deg threshold for contact detection
        ...
        rospy.loginfo(f"Initial valve yaw: {math.degrees(self.initial_valve_yaw):.1f} deg")
        rospy.loginfo(f"Contact detection threshold: {math.degrees(self.valve_yaw_change_threshold):.1f} deg")
    ...
    def execute_two_phase_insertion(self, params):
        ...
        rospy.loginfo(f"  Right claw target: ({phase1_right_safe[0]:.3f}, {phase1_right_safe[1]:.3f})")
        rospy.loginfo(f"  UAV position: ({phase1_uav_pos[0]:.3f}, {phase1_uav_pos[1]:.3f}, {phase1_uav_pos[2]:.3f})")
        rospy.loginfo(f"  UAV yaw: {math.degrees(phase1_uav_yaw):.1f} deg")
        ...
        rospy.loginfo(f"  UAV yaw: {math.degrees(phase2_uav_yaw):.1f} deg")
        ...
        rospy.loginfo("[OK] Phase 2 completed - Claws at final contact points (inner rim)")
        ...
        if final_pos:
            ...
            if success:
                rospy.loginfo("[OK] TWO-PHASE SAFE INSERTION SUCCESSFUL")
                rospy.loginfo(f"[OK] User-verified same-side strategy implemented: {same_side_result['formula']}")
                return True
            else:
    ...
    def calculate_uav_position_for_targets(self, left_target, right_target, target_z):
        ...
        try:
            ...
            rospy.loginfo(f"Valve yaw: {math.degrees(self.valve_yaw):.1f} deg")
            rospy.loginfo(f"Dual-fang center: ({dual_fang_center_x:.3f}, {dual_fang_center_y:.3f})")
            rospy.loginfo(f"Calculated UAV yaw: {math.degrees(base_uav_yaw):.1f} deg")
            ...
            rospy.loginfo(f"Safety status: {'[OK] SAFE' if min(left_clearance, right_clearance) > 0 else '[FAIL] COLLISION RISK'}")
            ...
            rospy.loginfo(f"Dual-fang center: ({dual_fang_center_x:.3f}, {dual_fang_center_y:.3f})")
            rospy.loginfo(f"UAV yaw: {math.degrees(uav_yaw):.1f} deg")
    ...
    def execute_safe_movement_to_position(self, target_pos, target_yaw):
        ...
        try:
            ...
            rospy.loginfo("[OK] Stage 1 completed - XY positioning done")
            ...
            if stage1_final_pos:
                ...
                if z_distance_remaining < 0.010:  # Less than 10mm remaining (relaxed from 5mm)
                    rospy.loginfo("[OK] INSERTION ALREADY COMPLETE - Z distance < 10mm")
                    rospy.loginfo("[OK] Skipping unnecessary Z descent to prevent oscillation")
                    rospy.loginfo("[OK] THREE-STAGE MOVEMENT COMPLETED SUCCESSFULLY (EARLY COMPLETION)")
                    return True
            ...
            rospy.loginfo("[OK] Stage 2 completed - Yaw adjustment done")
            ...
            rospy.loginfo("[OK] Stage 3 completed - Z descent done")
            rospy.loginfo("[OK] THREE-STAGE MOVEMENT COMPLETED SUCCESSFULLY")
            return True
    ...
    def execute_radial_approach_movement(self, target_pos, target_yaw):
        ...
        try:
            ...
            rospy.loginfo(f"Required movements - XY: {xy_distance*1000:.1f}mm, Z: {z_distance*1000:.1f}mm, Yaw: {math.degrees(yaw_change):.1f} deg")
            ...
            if is_simulation:
                ...
                yaw_threshold = 0.05236  # 3 deg for simulation (tightened)
            else:
                xy_threshold = 0.05   # 50mm for real machine
                yaw_threshold = 0.05236  # 3 deg for real machine (tightened)
            ...
            rospy.loginfo("[OK] RADIAL APPROACH MOVEMENT COMPLETED")
            return True
    ...
    def verify_two_phase_insertion_result(self, final_pos, left_target, right_target, same_side_result):
        ...
        try:
            ...
            rospy.loginfo(f"Final UAV yaw: {math.degrees(self.current_yaw):.1f} deg")
            ...
            if positioning_accurate and separation_adequate:
                rospy.loginfo("[OK] TWO-PHASE INSERTION VERIFICATION SUCCESSFUL")
                rospy.loginfo(f"[OK] User's same-side strategy achieved: {same_side_result['formula']}")
                return True
            else:
    ...
    def execute_xy_only_movement(self, start_pos, target_pos, locked_yaw):
        ...
        rospy.loginfo(f"Z locked at: {start_pos[2]:.3f}m, Yaw locked at: {math.degrees(locked_yaw):.1f} deg")
        ...
        if insertion_depth > 0.1:  # 已插入超过100mm
            ...
            yaw_tolerance = 0.05236 # 收紧yaw阈值到3.0 deg
        ...
        rospy.loginfo(f"Adaptive thresholds: pos={pos_tolerance*1000:.0f}mm, yaw={math.degrees(yaw_tolerance):.1f} deg ({tolerance_context})")
    ...
    def execute_yaw_adjustment_with_compensation(self, target_yaw):
        ...
        For large yaw changes (>45 deg), uses multi-step progression to minimize drift.
        ...
        rospy.loginfo(f"Yaw adjustment with compensation: {math.degrees(current_yaw):.1f} deg -> {math.degrees(target_yaw):.1f} deg")
        rospy.loginfo(f"Yaw change magnitude: {math.degrees(yaw_change_magnitude):.1f} deg")

        # Check if progressive adjustment is needed for large yaw changes
        if yaw_change_magnitude > math.radians(45):  # 45 deg threshold
    ...
    def execute_progressive_yaw_adjustment(self, start_yaw, target_yaw):
        ...
        # Calculate number of steps (max 20 deg per step for better precision)
        max_step_size = math.radians(20)  # Reduced from 30 deg to 20 deg
        num_steps = max(2, int(math.ceil(yaw_change_magnitude / max_step_size)))

        rospy.loginfo(f"Total yaw change: {math.degrees(yaw_change_magnitude):.1f} deg")
        rospy.loginfo(f"Progressive steps: {num_steps} steps")

        # Calculate step angles
        step_size = total_yaw_change / num_steps
        rospy.loginfo(f"Step size: {math.degrees(step_size):.1f} deg per step")
        ...
        for step in range(num_steps):
            ...
            rospy.loginfo(f"--- Step {step + 1}/{num_steps}: {math.degrees(current_yaw):.1f} deg -> {math.degrees(step_target_yaw):.1f} deg ---")
        ...
        if total_drift < 0.020:  # Relaxed tolerance from 15mm to 20mm for total drift
            rospy.loginfo("[OK] Progressive yaw adjustment successful with acceptable drift")
            return True
        else:
    ...
    def execute_realtime_yaw_with_endeffector_lock(self, start_yaw, target_yaw,
        ...
        Returns:
            bool: Success status
        """
        rospy.loginfo(f"{step_prefix}Real-time yaw adjustment: {math.degrees(start_yaw):.1f} deg -> {math.degrees(target_yaw):.1f} deg")
        ...
        rospy.loginfo(f"{step_prefix}Rotation duration: {duration:.1f}s for {math.degrees(yaw_change_magnitude):.1f} deg")
        ...
        if final_pos:
            ...
            rospy.loginfo(f"{step_prefix}Final yaw: {math.degrees(final_yaw):.1f} deg")
            ...
            if ee_drift < 0.025:  # 25mm tolerance (relaxed from 8mm for realistic insertion)
                rospy.loginfo(f"[OK] {step_prefix}Real-time yaw adjustment successful")
                return True
            else:
        ...
        if compensation_distance > 0.001:  # 1mm threshold
            ...
            rospy.loginfo(f"{step_prefix}Duration: {duration:.1f}s for {math.degrees(yaw_change_magnitude):.1f} deg + {compensation_distance*1000:.1f}mm compensation")

            success = self.motion_controller.execute_progressive_precision_trajectory(
                current_pos, compensated_pos, target_yaw,
                final_pos_threshold=0.025,  # 25mm final precision (avoid oscillation)
                final_yaw_threshold=0.05    # 2.9 deg final precision
            )
            ...
            if success:
                ...
                if end_effector_drift > 0.015 and end_effector_drift < 0.030:  # 15-30mm range
                    ...
                    correction_success = self.motion_controller.execute_progressive_precision_trajectory(
                        ...
                        final_yaw_threshold=0.03    # 1.7 deg final precision
                    )
                ...
                if end_effector_drift < 0.015:  # Relaxed tolerance from 12mm to 15mm
                    rospy.loginfo(f"[OK] {step_prefix}Yaw adjustment step successful")
                    return True
                else:
    ...
    def execute_pure_yaw_rotation(self, target_yaw):
        ...
        while (time.time() - start_time) < 8.0 and not rospy.is_shutdown():
            ...
            if yaw_error < 0.03:  # 0.03 rad = ~1.7 deg
                rospy.loginfo(f"Pure yaw rotation completed. Error: {math.degrees(yaw_error):.1f} deg")
                return True
    ...
    def execute_z_only_movement(self, start_pos, target_pos):
        ...
        rospy.loginfo(f"Yaw locked at: {math.degrees(self.current_yaw):.1f} deg")
        ...
        rospy.loginfo(f"Control mode: {mode_context}, pos_threshold: {pos_threshold*1000:.0f}mm, yaw_threshold: {math.degrees(yaw_threshold):.1f} deg")
    ...
    def execute_legacy_insertion(self, params):
        ...
        rospy.loginfo(f"Target UAV yaw: {math.degrees(optimal_uav_yaw):.1f} deg")
...
class RotateValveState(SingleUAVStateBase):
    def __init__(self, module_id=1):
        ...
        self.circular_motion_step = math.radians(5.0)  # 5 deg incremental steps
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo(f"初始阀门角度: {math.degrees(initial_valve_yaw):.1f} deg")
        rospy.loginfo(f"接触检测阈值: {math.degrees(valve_yaw_threshold):.1f} deg")
        ...
        if hasattr(userdata, 'trajectory_state') and userdata.trajectory_state is not None:
            ...
            trajectory_generator = OnlineCircularTrajectoryGenerator(
                ...
                target_angular_velocity=0.1,  # 修复：0.1 rad/s ~ 5.7 deg/s（Manipulate阶段保持足够转动力矩）
            ...
            rospy.loginfo("[OK] 基于Contact状态重建轨迹生成器，确保连续性")
            rospy.loginfo(f"[OK] 状态恢复: 半径={traj_state['initial_radius']*1000:.1f}mm, 锁定={traj_state.get('radius_locked', False)}")
        else:
            # 后备方案：全新创建
            ...
            trajectory_generator = OnlineCircularTrajectoryGenerator(
                ...
                target_angular_velocity=0.1,  # 修复：0.1 rad/s ~ 5.7 deg/s（后备方案保持相同速度）
        ...
        # Dragon Manipulate参数
        target_valve_rotation = math.radians(90.0)  # 目标阀门转动角度：90 deg（典型阀门1/4圈）
        ...
        if hasattr(userdata, 'contact_final_torque') and userdata.contact_final_torque is not None:
            ...
            rospy.loginfo(f"[OK] 接力使用Contact最终扭矩: {baseline_torque}")
        else:
        ...
        rospy.loginfo(f"目标: {math.degrees(target_valve_rotation):.1f} deg阀门转动")
        ...
        while (rospy.Time.now().to_sec() - start_time) < max_manipulation_time:
            ...
            if current_time > last_valve_check_time + 0.2:  # 每200ms更新一次
                ...
                # 检测阀门运动停滞
                if valve_angular_velocity < 0.01:  # < 0.6 deg/s认为停滞
            ...
            try:
                ...
                if not isinstance(trajectory_generator, OnlineCircularTrajectoryGenerator):
                    ...
                    trajectory_generator = OnlineCircularTrajectoryGenerator(
                        valve_center=corrected_valve_center,
                        initial_radius=initial_radius,
                        target_angular_velocity=0.1,  # 修复：0.1 rad/s ~ 5.7 deg/s（错误恢复保持相同速度）
                        control_rate=25.0,  # 优化：25Hz
                        debug=True
                    )
                    # 重要：重新创建后必须初始化
                ...
                if not callable(getattr(trajectory_generator, 'update_state', None)):
                    ...
                    trajectory_generator = OnlineCircularTrajectoryGenerator(
                        valve_center=corrected_valve_center,
                        initial_radius=initial_radius,
                        target_angular_velocity=0.05,  # 统一：0.05 rad/s ~ 2.9 deg/s
                        control_rate=25.0,  # 优化：25Hz
                        debug=True
                    )
                    # 重要：重新创建后必须初始化
            ...
            if valve_rotation >= target_valve_rotation:
                ...
                if elapsed_time > min_manipulation_time:  # 确保最小操作时间
                    rospy.loginfo(f"[OK] Manipulate成功! 阀门转动: {math.degrees(valve_rotation):.1f} deg")
                    rospy.loginfo(f"操作时间: {elapsed_time:.1f}s")
                    rospy.loginfo(f"最终角速度: {math.degrees(state_info['angular_velocity']):.1f} deg/s")
            ...
            if valve_motion_stalled_time > valve_stuck_threshold:
                ...
                if max_valve_rotation_achieved > target_valve_rotation * 0.7:  # 达到70%认为部分成功
                    rospy.loginfo(f"部分成功：已转动 {math.degrees(max_valve_rotation_achieved):.1f} deg，继续尝试")
            ...
            if current_time - start_time > 2.0:  # 2秒后开始输出调试信息
                ...
                rospy.loginfo_throttle(3.0,
                    f"Manipulate状态: 阀门转动={math.degrees(valve_rotation):.1f} deg/"
                    f"{math.degrees(target_valve_rotation):.1f} deg, "
                    f"阀门角速度={math.degrees(valve_angular_velocity):.1f} deg/s, "
                    f"UAV角速度={status['current_angular_velocity_deg']:.1f} deg/s, "
        ...
        if final_valve_rotation >= target_valve_rotation * 0.8:  # 80%认为可接受
            rospy.logwarn(f"Manipulate超时，但达到可接受转动: {math.degrees(final_valve_rotation):.1f} deg")
            ...
            rospy.logerr(f"Manipulate失败：仅转动 {math.degrees(final_valve_rotation):.1f} deg/"
                        f"{math.degrees(target_valve_rotation):.1f} deg")
            return 'failed'
...
class DisengageFromValveState(SingleUAVStateBase):
    ...
    def execute(self, userdata):
        ...
        1.5. 反向旋转脱离：反向旋转20 deg释放机械约束，防止卡住
        ...
        rospy.loginfo("阶段: 松弛 -> 反向旋转脱离(15 deg) -> 垂直抬高 -> 水平脱离 -> 安全位置")
    ...
    def _execute_reverse_rotation_phase(self, current_pos, current_yaw, valve_pos, trajectory_state):
        ...
        if trajectory_state and 'generator' in trajectory_state:
            ...
            rospy.loginfo(f"[OK] 继承ROTATE_VALVE轨迹状态，实现连续反向旋转")
            rospy.loginfo(f"[OK] 阀门中心: ({corrected_valve_center[0]:.3f}, {corrected_valve_center[1]:.3f}, {corrected_valve_center[2]:.3f})")
            rospy.loginfo(f"[OK] 反向角速度: {math.degrees(trajectory_generator.target_angular_velocity):.1f} deg/s")
            rospy.loginfo(f"[OK] 当前半径: {trajectory_generator.current_radius*1000:.1f}mm")
        else:
            # 后备方案：创建新的轨迹生成器
            ...
            try:
                trajectory_generator = OnlineCircularTrajectoryGenerator(
                    valve_center=corrected_valve_center,
                    initial_radius=initial_radius,
                    target_angular_velocity=-0.087,  # 反向旋转，约-5 deg/s
                    control_rate=25.0,
                    debug=True
                )
                # 重要：基于当前位置初始化角度
        ...
        rospy.loginfo(f"开始反向旋转: 目标角度={math.degrees(target_reverse_angle):.1f} deg")
        rospy.loginfo(f"反向角速度: {math.degrees(-0.087):.1f} deg/s")

        # 基于UAV实际yaw角度的反向旋转控制（更准确、更温和）
        target_angular_speed = math.radians(2.0)  # 减慢到2 deg/s，更温和的反向旋转

        # 记录初始yaw角度作为参考
        initial_yaw = current_yaw
        rospy.loginfo(f"初始偏航角: {math.degrees(initial_yaw):.1f} deg")
        rospy.loginfo(f"目标反向旋转: {math.degrees(target_reverse_angle):.1f} deg")
        rospy.loginfo(f"温和角速度: {math.degrees(target_angular_speed):.1f} deg/s")
        ...
        while not rospy.is_shutdown():
            ...
            if accumulated_yaw_change >= target_reverse_angle:
                rospy.loginfo(f"[OK] 反向旋转完成: 实际UAV yaw变化 {math.degrees(accumulated_yaw_change):.1f} deg")
                break
            ...
            if int(elapsed_time) != int(elapsed_time - 0.04):
                ...
                rospy.loginfo(f"反向旋转进度: 实际yaw变化 {math.degrees(accumulated_yaw_change):.1f} deg/{math.degrees(target_reverse_angle):.1f} deg ({progress:.0f}%), 时间: {elapsed_time:.1f}s")

            control_rate.sleep()

        # 反向旋转完成，充足稳定时间
        final_time = rospy.Time.now().to_sec() - start_time
        rospy.loginfo(f"反向旋转完成: 实际UAV yaw变化 {math.degrees(accumulated_yaw_change):.1f} deg (用时: {final_time:.1f}s)")
        ...
        rospy.loginfo(f"[OK] 稳定阶段完成，机械脱离充分完成")
        ...
        if accumulated_yaw_change >= target_reverse_angle * 0.6:  # 60%角度变化认为成功（更宽容）
            rospy.loginfo(f"[OK] 反向旋转脱离成功: 实际 {actual_rotation_deg:.1f} deg (目标: {target_rotation_deg:.1f} deg)")
            return 'success'
        else:
            rospy.logwarn(f"反向旋转角度不足: {actual_rotation_deg:.1f} deg < {target_rotation_deg * 0.6:.1f} deg，但继续执行")
        ...
        if start_z_height > current_z + 0.01:  # 如果目标高度比当前高度高1cm以上
            ...
            if final_pos_check is not None:
                ...
                if actual_z_error < 0.05:  # 5cm误差容忍
                    rospy.loginfo(f"[OK] 分段Z上升基本完成，Z误差={actual_z_error*1000:.1f}mm")
                else:
                    rospy.logwarn(f"分段Z上升精度不足，Z误差={actual_z_error*1000:.1f}mm，但继续执行")
            else:
                rospy.loginfo("[OK] 分段Z上升过程完成，继续后续阶段")
        else:
    ...
    def _execute_yaw_adjustment_to_start_angle(self, current_pos, current_yaw, userdata):
        """
        阶段3: Yaw调整到任务开始时的角度
        调整到0 deg偏航角（任务开始的标准角度）
        """
        rospy.loginfo(">>> 阶段3: Yaw调整到任务开始角度")

        # 目标偏航角：0 deg（任务开始的标准角度）
        target_yaw = 0.0
        ...
        rospy.loginfo(f"Yaw调整角度: {math.degrees(yaw_diff):.1f} deg")

        # 如果偏航差值较大才执行调整
        if abs(yaw_diff) > math.radians(5.0):  # 5 deg阈值
            success = self.beetle.goPoseWaitConvergence(
                pos=current_pos,  # 保持当前XYZ位置
                rot=target_yaw,   # 调整到目标偏航
                pos_thresh=0.02,  # 2cm位置精度
                rot_thresh=0.0873,  # 5 deg偏航精度
                timeout=10.0
            )
            ...
            if not success:
                ...
                rospy.loginfo("[OK] Yaw调整完成，已回到任务开始角度")
        else:
    ...
    def _execute_xy_return_to_start_position(self, current_pos, current_yaw, userdata):
        ...
        rospy.loginfo(f"保持偏航: {math.degrees(current_yaw):.1f} deg")
...
class HorizontalReturnState(SingleUAVStateBase):
    ...
    def execute(self, userdata):
        ...
        target_pos = (start_position[0], start_position[1], current_pos[2])
        target_yaw = 0.0  # Return to 0 deg yaw
        ...
        success = self.beetle.goPoseWaitConvergence(
            ...
            rot_thresh=math.radians(5.0),  # 5 deg yaw tolerance
        ...
        rospy.loginfo("[OK] HORIZONTAL RETURN COMPLETED")
        return 'succeeded'
...
def main():
    ...
    try:
        ...
        convergence_params = {
            ...
            'rot_thresh': rospy.get_param('~convergence/rot_thresh', 0.0524),  # rad (3 deg)
