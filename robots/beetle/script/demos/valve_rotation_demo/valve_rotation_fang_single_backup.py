...
class RotateValveState(SingleUAVStateBase):
    ...
    def execute(self, userdata):
        ...
        # Check if trajectory creation failed
        if rotation_traj is None:
            rospy.logerr("Failed to create rotation trajectory - UAV position validation failed")
            rospy.logerr("UAV is too close to valve center for safe rotation")
            rospy.logerr("Please re-execute insertion process to correct UAV position")
            return 'failed'
