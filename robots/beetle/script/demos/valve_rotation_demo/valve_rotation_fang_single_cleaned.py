...
class DescendAndContactState(SingleUAVStateBase):
    ...
    def adjust_yaw_and_circumferential_direction(self):
        ...
        # Move 15mm closer to valve center
        ...
        # Move 20mm closer to valve center (increased from 15mm)
    ...
    def execute_smooth_trajectory(self, start_pos, target_pos, target_yaw, duration=5.0):
        """Execute smooth trajectory with ABSOLUTE Z-lock to prevent oscillation"""
        ...
        rospy.loginfo(f"ABSOLUTE Z-lock trajectory:")
        ...
        # Execute trajectory with absolutely locked Z
        start_time = time.time()
        rate = rospy.Rate(30)  # Reduced frequency for stability
        ...
        while not rospy.is_shutdown() and (time.time() - start_time) < duration + 2.0:
            ...
            current_traj_pos = traj.get_next_position()
            if current_traj_pos is None:
                current_traj_pos = locked_target_pos
            
            # CRITICAL: Force Z to be absolutely locked
            locked_position = (current_traj_pos[0], current_traj_pos[1], locked_z)
            
            # Calculate yaw
            current_yaw = start_yaw + progress * yaw_diff
            
            # Send command with absolutely locked Z
            self.motion_controller.send_trajectory_point(locked_position, current_yaw)
            
            # Check convergence using only XY distance
            current_pos = self.get_current_position()
            if current_pos is not None:
                ...
                if xy_distance < 0.03 and yaw_error < 0.08 and progress >= 0.9:
                    rospy.loginfo(f"Trajectory converged: XY={xy_distance:.3f}m, yaw={yaw_error:.3f}rad")
                    break
            
            rate.sleep()
        
        rospy.loginfo("Absolutely Z-locked trajectory completed")
        return True
